...
export async function GET(request: NextRequest) {
  try {
    ...
    const where = invoiceId ? "AND dnh.invoice_id = $2" : ""
    if (invoiceId) params.push(invoiceId)
    const res = await query(
      `SELECT dnh.*, ih.invoice_number, c.client_name
       FROM debit_note_header dnh
       JOIN invoice_header ih
         ON ih.id = dnh.invoice_id
        AND ih.company_id = dnh.company_id
       JOIN clients c
         ON c.id = dnh.client_id
        AND c.company_id = dnh.company_id
       WHERE dnh.company_id = $1
       ${where}
       ORDER BY dnh.note_date DESC, dnh.id DESC`,
      params
    )
    return ok(res.rows)
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : "Failed to fetch debit notes"
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const routeKey = `finance.debit-notes.create:${payload.invoice_id}:${payload.note_date}`
    if (idempotencyKey) {
    ...
    const noteNumber = makeNoteNumber("DN", seq, payload.note_date)
    ...
    const header = headerRes.rows[0] as DebitNoteHeaderRow

    let lineNo = 1
    for (const line of mapped) {
      await db.query(
        `INSERT INTO debit_note_lines (
           company_id, debit_note_id, invoice_line_id, line_no, description, quantity, rate, amount, tax_rate, tax_amount, gross_amount
         ) VALUES (
    ...
    await db.query(
      `UPDATE invoice_header
       SET balance_amount = balance_amount + $1,
           status = CASE
             WHEN status = 'PAID' THEN 'FINALIZED'
             ELSE status
           END,
           updated_by = $2,
    ...
    return ok(header, "Debit note issued")
  } catch (error: unknown) {
    await db.query("ROLLBACK")
